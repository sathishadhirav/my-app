...
public class MT5API {
	...
	public ArrayList<String> LoginIdExServerUrls = new ArrayList<String>(Arrays.asList("http://loginid-mt5.mtapi.io"));
	 ...
	 * Manually or EA
	 */
	public PlacedType PlacedType = io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.PlacedType.Manually;
	...
	public final void UpdateProfits() throws IOException {
		for (io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.Order item : GetOpenedOrders()) {
			if (item.OrderType == OrderType.Buy || item.OrderType == OrderType.Sell) {
				...
				io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.Quote quote = GetQuote(item.Symbol);
	...
	public final double AccountMargin() throws IOException {
		...
		for (io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.Order order : GetOpenedOrders()) {
	 ...
	 *
	 * @param requestId  Uniq temporary ID that can be used before ticket would be assigned. You can use GetRequestID()
	 * @param symbol     Symbol
	 * @param lots       Lots
	 * @param price      Price
	...
	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, sl, tp, deviation, comment, expertID, fillPolicy, 0);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, sl, tp, deviation, comment, expertID, FillPolicy.FillOrKill, 0);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, sl, tp, deviation, comment, 0, FillPolicy.FillOrKill, 0);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, sl, tp, deviation, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl, double tp) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, sl, tp, 0, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, sl, 0, 0, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, 0, 0, 0, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID,
									 FillPolicy fillPolicy, double stoplimit) throws IOException {
		OrderSendAsync(requestId, symbol, lots, price, type, sl, tp, deviation, comment, expertID, fillPolicy, stoplimit, null);
	}

	public final void OrderSendAsync(int requestId, String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID,
									 FillPolicy fillPolicy, double stoplimit, LocalDateTime expiration) throws IOException {
	...
	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy) throws IOException {
		return OrderSend(symbol, lots, price, type, sl, tp, deviation, comment, expertID, fillPolicy, 0);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy, double stoplimit) throws IOException {
		return OrderSend(symbol, lots, price, type, sl, tp, deviation, comment, expertID, fillPolicy, stoplimit, null);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID) throws IOException {
		return OrderSend(symbol, lots, price, type, sl, tp, deviation, comment, expertID, FillPolicy.FillOrKill, 0);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment) throws IOException {
		return OrderSend(symbol, lots, price, type, sl, tp, deviation, comment, 0, FillPolicy.FillOrKill, 0);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation) throws IOException {
		return OrderSend(symbol, lots, price, type, sl, tp, deviation, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl, double tp) throws IOException {
		return OrderSend(symbol, lots, price, type, sl, tp, 0, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl) throws IOException {
		return OrderSend(symbol, lots, price, type, sl, 0, 0, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type) throws IOException {
		return OrderSend(symbol, lots, price, type, 0, 0, 0, null, 0, FillPolicy.FillOrKill, 0);
	}

	public final Order OrderSend(String symbol, double lots, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy, double stoplimit, LocalDateTime expiration) throws IOException {
		int id = GetRequestId();
		Order order;
		if (type == OrderType.Buy || type == OrderType.Sell) {
			MarketOpenWaiter waiter = new MarketOpenWaiter(this, id, ExecutionTimeout);
			OrderSendAsync(id, symbol, lots, price, type, sl, tp, deviation, comment, expertID, fillPolicy, stoplimit, expiration);
			order = waiter.Wait();
		} else {
		...
		if (AccountMethod() == AccMethod.Hedging) {
			...
			if (value != null)
				...
				Orders.Opened.put(order.Ticket, order);
		}
		return order;
	}
	 ...
	 *
	 * @param requestId Uniq temporary ID that can be used before ticket would be assigned. You can use GetID()
	...
	public final void OrderCloseAsync(int requestId, long ticket, String symbol, double price, double lots, OrderType type, long deviation) throws IOException {
		OrderCloseAsync(requestId, ticket, symbol, price, lots, type, deviation, FillPolicy.FillOrKill);
	}

	public final void OrderCloseAsync(int requestId, long ticket, String symbol, double price, double lots, OrderType type) throws IOException {
		OrderCloseAsync(requestId, ticket, symbol, price, lots, type, 0, FillPolicy.FillOrKill);
	}

	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
//ORIGINAL LINE: public void OrderCloseAsync(int requestId, long ticket, string symbol, double price, double lots, OrderType type, ulong deviation = 0, FillPolicy fillPolicy = FillPolicy.FillOrKill)
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
	public final void OrderCloseAsync(int requestId, long ticket, String symbol, double price, double lots, OrderType type, long deviation, FillPolicy fillPolicy) throws IOException {
...
//ORIGINAL LINE: req.Lots = (ulong)(lots * 100000000);
	...
	public final Order OrderClose(long ticket, String symbol, double price, double lots, OrderType type, long deviation) throws IOException {
		return OrderClose(ticket, symbol, price, lots, type, deviation, FillPolicy.FillOrKill);
	}

	public final Order OrderClose(long ticket, String symbol, double price, double lots, OrderType type) throws IOException {
		return OrderClose(ticket, symbol, price, lots, type, 0, FillPolicy.FillOrKill);
	}

	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
//ORIGINAL LINE: public Order OrderClose(long ticket, string symbol, double price, double lots, OrderType type, ulong deviation = 0, FillPolicy fillPolicy = FillPolicy.FillOrKill)
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
	public final Order OrderClose(long ticket, String symbol, double price, double lots, OrderType type, long deviation, FillPolicy fillPolicy) throws IOException {
		Order order;
		if (type == OrderType.Buy || type == OrderType.Sell) {
			int id = GetRequestId();
	...
	public final void OrderModify(long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy) throws IOException {
		OrderModify(ticket, symbol, volume, price, type, sl, tp, deviation, comment, expertID, fillPolicy, 0);
	}

	public final void OrderModify(long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID) throws IOException {
		OrderModify(ticket, symbol, volume, price, type, sl, tp, deviation, comment, expertID, FillPolicy.FillOrKill, 0);
	}

	public final void OrderModify(long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation, String comment) throws IOException {
		OrderModify(ticket, symbol, volume, price, type, sl, tp, deviation, comment, 0, FillPolicy.FillOrKill, 0);
	}

	public final void OrderModify(long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation) throws IOException {
		OrderModify(ticket, symbol, volume, price, type, sl, tp, deviation, "", 0, FillPolicy.FillOrKill, 0);
	}

	public final void OrderModify(long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp) throws IOException {
		OrderModify(ticket, symbol, volume, price, type, sl, tp, 0, "", 0, FillPolicy.FillOrKill, 0);
	}

	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
//ORIGINAL LINE: public void OrderModify(long ticket, string symbol, double volume, double price, OrderType type, double sl, double tp, ulong deviation = 0, string comment = "", long expertID = 0, FillPolicy fillPolicy = FillPolicy.FillOrKill, double stoplimit = 0)
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
	public final void OrderModify(long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy, double stoplimit) throws IOException {
		int id = GetRequestId();
		ModifyWaiter waiter = new ModifyWaiter(this, id, ExecutionTimeout, ticket);
		OrderModifyAsync(id, ticket, symbol, volume, price, type, sl, tp, deviation, comment, expertID, fillPolicy, stoplimit);
		waiter.Wait();
	}


	public final void OrderModifyAsync(int requestId, long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy) throws IOException {
		OrderModifyAsync(requestId, ticket, symbol, volume, price, type, sl, tp, deviation, comment, expertID, fillPolicy, 0);
	}

	public final void OrderModifyAsync(int requestId, long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID) throws IOException {
		OrderModifyAsync(requestId, ticket, symbol, volume, price, type, sl, tp, deviation, comment, expertID, FillPolicy.FillOrKill, 0);
	}

	public final void OrderModifyAsync(int requestId, long ticket, String symbol, double volume, double price, OrderType type, double sl, double tp, long deviation, String comment, long expertID, FillPolicy fillPolicy, double stoplimit) throws IOException {
		TradeRequest req = new TradeRequest();
		req.PlacedType = PlacedType;
	...
	public final boolean Connected() {
		...
		if (!Connection.Sock.isConnected())
			return false;
		//if (Duration.between(CmdHandler.LastMessageFromServer, LocalDateTime.now()).toMillis() > LastMessageFromServerTimeoutMs)
		//	return false;
...
//#endif
	...
	public MT5API(long user, String password, String host, int port, byte[] pfxFile, String pfxFilePassword) {
		Trial.check(guid);
		Log = new Logger(this);
		User = user;
		Password = password;
	 ...
	 */
	public final void RequestDealHistory(java.time.LocalDateTime from, java.time.LocalDateTime to) throws IOException {
	 ...
	 * @return Array of orders.
	 */
	public void RequestDealHistory(int year, int month, List<DealInternal> exist) throws ConnectException, IOException {
	 ...
	 * @return Array of orders.
	 */
	public final void RequestOrderHistory(java.time.LocalDateTime from, java.time.LocalDateTime to) throws IOException {
		Log.trace("RequestOrderHistory");
		if (!Connected()) {
			throw new RuntimeException("Not connected");
		}
	 ...
	 * @return Array of orders.
	 */
	public void RequestOrderHistory(int year, int month, List<OrderInternal> exist) throws IOException, ConnectException {
		Log.trace("RequestOrderHistoryMonth");
		if (!Connected())
			Connect();
		LocalDateTime start = LocalDateTime.of(year, month, 1, 0, 0);
	 ...
	 *
	 * @param symbol Symbol for trading.
	 * @return Return null if no quotes for specified symbol avalible, otherwise return quote event arguments.
	 * @throws Exception Symbol not subscribed.
	 */
	public final Quote GetQuote(String symbol) throws IOException {
		io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.Quote q = Subscriber.GetQuote(symbol);
		if (q != null) {
			if (q.Bid == 0 || q.Ask == 0) {
				return null;
			}
	 ...
	 * @throws Exception Symbol not subscribed.
	 */
	public Quote GetQuote(String symbol, int msTimeout) throws IOException, TimeoutException, ConnectException {
		if(!Connected())
			Connect();
		if (!Symbols.Exist(symbol))
			throw new RuntimeException("Symbol " + symbol + " not exist");
	 ...
	 * @param day
	 */
	public void RequestQuoteHistoryMonth(String symbol, int year, int month, int day) throws IOException {
		if (!Symbols.Exist(symbol))
