...
class Connector
{
	...
	public final void go(int msTimeout, String host, int port) throws Exception {
		...
		while(true) {
			...
			synchronized (ConnectLock) {
				...
				ConnectThread = new MyThread(runnable);
		...
		if (ConnectThread.Join(msTimeout)) {
			if (ConnectThread.Exception == null) {
				LinkedList<String> subscriptions = new LinkedList<String>(Arrays.asList(QC.Subscriptions()));
	...
	private void ConnectToAccount() throws Exception {
		...
		while (QC.GotAccountInfo == false) {
			...
			if (cmd.AccountLoaderException != null)
				throw cmd.AccountLoaderException;
			Thread.sleep(1);
		}
	}
}