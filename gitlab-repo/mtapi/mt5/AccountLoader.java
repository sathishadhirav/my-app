...
//ORIGINAL LINE: var cmd = buf.Byte();
				...
				switch ((cmd & 0xFF))
				{
					...
					case 0x18: //mail recepients
						io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.MailRecipient[] mr = LoadMailRecepients(buf);
						break;
					case 0x1F: //order
	...
	private void LoadSpreads(InBuf buf)
	{
		...
		Msg status = Msg.forValue(buf.Int());
		if(status == null)
			throw new RuntimeException("LoadSpreads status is null");;
		if (status == Msg.OK)
		{
			return;
		}
	...
	private Map.Entry<ServerRec, ArrayList<Map.Entry<AccessInfo, AddressRec[]>>> LoadServer(InBuf buf)
	{
		Log.trace("LoadServer");
		ServerRec sr = buf.Struct(new ServerRec());
		int num = buf.Int();
		ArrayList<Map.Entry<AccessInfo, AddressRec[]>> list = new ArrayList<Map.Entry<AccessInfo, AddressRec[]>>();
		for (int i = 0; i < num; i++)
...
//		if (QuoteClient.Symbols.Groups.values().size() < 100 && QuoteClient.Symbols.SymGroups.length < 100)
...
//			}
	...
	private void LoadSymBase(InBuf buf) throws ConnectException {
		...
		if (build <= 1495)
			throw new ConnectException("SymBuild: " + Connection.SymBuild);//return LoadBuild1495(pBufMan);
		if (build <= 1613)
			throw new ConnectException("SymBuild: " + Connection.SymBuild); //return LoadBuild1613(pBufMan);
		if (build <= 1891)
			throw new ConnectException("SymBuild: " + Connection.SymBuild); //return LoadBuild1891(pBufMan);
		if (build <= 2017)
		{
			//LoadBuild2017(buf);
