...
//C# TO JAVA CONVERTER TODO TASK: There is no equivalent to implicit typing in Java:
		...
		{
			io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.SymbolInfo si = QuoteClient.Symbols.GetInfo(item);
...
//ORIGINAL LINE: if (!br.GetULong(out volumeEx))
			...
			io.quantum.trading.brokers.forex.mt5protocol.mtapi.mt5.SymbolInfo info = QuoteClient.Symbols.GetInfo(rec.Id);
			Quote quote;
	...
	public void ParseSymbolData(InBuf buf)
	{
		...
		while (true)
		{
			...
			if (!br.GetULong(lr))
				...
				rec.UpdateMask = lr.argValue;
			if (!br.GetLong(lr))
				break;
			else
				rec.Time = lr.argValue;
			if ((rec.UpdateMask & 1) != 0)
				if (!br.GetLong(lr))
					break;
				else
			...
			if ((rec.UpdateMask & 2) != 0)
				if (!br.GetLong(lr))
					...
					rec.BidHigh = lr.argValue;
			if ((rec.UpdateMask & 4) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.BidLow = lr.argValue;
			if ((rec.UpdateMask & 8) != 0)
				if (!br.GetLong(lr))
					break;
				else
			...
			if ((rec.UpdateMask & 0x10) != 0)
				if (!br.GetLong(lr))
					...
					rec.AskHigh = lr.argValue;
			if ((rec.UpdateMask & 0x20) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.AskLow = lr.argValue;
			if ((rec.UpdateMask & 0x40) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.Last = lr.argValue;
			if ((rec.UpdateMask & 0x80) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.LastHigh = lr.argValue;
			if ((rec.UpdateMask & 0x100) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.LastLow = lr.argValue;
			if ((rec.UpdateMask & 0x200) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.Volume = lr.argValue;
			if ((rec.UpdateMask & 0x400) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.VolumeHigh = lr.argValue;
			if ((rec.UpdateMask & 0x800) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.VolumeLow = lr.argValue;
			if ((rec.UpdateMask & 0x1000) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.BuyVolume = lr.argValue;
			if ((rec.UpdateMask & 0x2000) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.SellVolume = lr.argValue;
			if ((rec.UpdateMask & 0x4000) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.BuyOrders = lr.argValue;
			if ((rec.UpdateMask & 0x8000) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.SellOrders = lr.argValue;
			if ((rec.UpdateMask & 0x10000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.Deals = lr.argValue;
			if ((rec.UpdateMask & 0x20000) != 0)
				if (!br.GetULong(lr))
					break;
				else
					rec.DealsVolume = lr.argValue;
			if ((rec.UpdateMask & 0x40000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.Turnover = lr.argValue;
			if ((rec.UpdateMask & 0x80000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.OpenInterest = lr.argValue;
			if ((rec.UpdateMask & 0x100000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.OpenPrice = lr.argValue;
			if ((rec.UpdateMask & 0x200000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.ClosePrice = lr.argValue;
			if ((rec.UpdateMask & 0x400000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.AverageWeightPrice = lr.argValue;
			if ((rec.UpdateMask & 0x800000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.PriceChange = lr.argValue;
			if ((rec.UpdateMask & 0x1000000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.PriceVolatility = lr.argValue;
			if ((rec.UpdateMask & 0x2000000) != 0)
				if (!br.GetLong(lr))
					break;
				else
					rec.PriceTheoretical = lr.argValue;
			...
			if ((rec.UpdateMask & 0x100000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x200000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x400000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x800000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x1000000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x2000000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x4000000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x8000000000L) != 0)
				if (!br.GetLong(lr))
					break;

			if ((rec.UpdateMask & 0x10000000000L) != 0)
				if (!br.GetLong(lr))
					break;
